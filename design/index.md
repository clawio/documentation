...
ClawIO architecture is also similar to the architecture specification found on the [IETF Internet Sync Storage draft](https://datatracker.ietf.org/doc/draft-cui-iss-problem/?include_text=1).
...
ClawIO is shipped with an Authentication Unit that authenticates users against a SQLite3 database.
...
*Further developments will include using [OpenStack Swift](http://docs.openstack.org/developer/swift/)/[Amazon S3](https://aws.amazon.com/s3/) and [Ceph RadosGW]() as data units*.
...
With this design is possible to offer end users **Direct Access to the Storage**, so changes made via third-party tools to the filesystem will be presented to the user in real time. This approach reduces the load on the database and increases the filesystem IO.

There is an alternative when ClawIO is deployed on top of a filesystem with support for extended attributes like EXT4. These filesystems can attach metadata to a file, so file ids can be kept along the file in the most **atomic** possible way. The SQL database is just delegated to a pseudo-KV store. This design is inspired by [EOS](http://eos.web.cern.ch/content/about-eos), the underlying data backend that powers CERNBox.
...
The Sync Protocol shipped with ClawIO is the ownCloud Sync Protocol. [ownCloud clients](https://owncloud.org/install/#install-clients) like the Desktop Sync Client can be connected to the ClawIO server to experiment different data backends.
To ensure the correctness of the protocol implementation, the test suite [SmashBox](https://github.com/cernbox/smashbox) is run.

*Further developments will include the SeaFile sync protocol. The choice of the SeaFile sync protocol among others is because its implementation is radically different from ownCloud. ClawIO could lead to the standard sync specification ISS researchers are trying to create.*